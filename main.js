...
var activityInterval = null, clockInterval = null, statsInterval = null;
var pendingRequests = {};
...
function findUser(username) {
    return allUsers.find(function(u) { return u.username.toLowerCase() === username.toLowerCase(); });
}
...
async function login() {
    ...
    try {
        ...
        if (r && r.email === email && r.password === pass) {
            ...
            document.getElementById('mainContainer').style.maxWidth = '800px';
            ...
            await loadUsers(); updateStats(); loadActivity();
        } else {
...
function logout() {
    currentAdmin = null; stopBackgroundTasks();
    if (sessionTimer) clearTimeout(sessionTimer);
...
function startBackgroundTasks() {
    ...
    activityInterval = setInterval(function() { if (currentAdmin) loadActivity(); }, 10000);
}
function stopBackgroundTasks() { if (clockInterval) clearInterval(clockInterval); if (statsInterval) clearInterval(statsInterval); if (activityInterval) clearInterval(activityInterval); }
...
async function loadActivity() {
    try {
        ...
        if (!logs.length) { c.innerHTML = '<div class="empty-state">Belum ada aktivitas</div>'; return; }
        var h = '';
        logs.forEach(function(l) {
            var time = l.timestamp ? new Date(l.timestamp).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) : '-';
            var actionLabel = { login: 'Login', topup: 'Top Up', kuras: 'Kuras', gantinama: 'Ganti Nama', banned: 'Ban', unbanned: 'Unban', force_logout: 'Force Logout', unforce_logout: 'Izinkan Login', deleted: 'Hapus' }[l.action] || l.action;
            h += '<div class="activity-item"><div class="activity-dot ' + (l.action || '') + '"></div><div class="activity-info"><span class="activity-user">' + esc(l.username) + '</span> <span class="activity-desc">' + actionLabel + (l.details ? ' — ' + l.details : '') + '</span></div><div class="activity-time">' + time + '</div></div>';
        });
        c.innerHTML = h;
    } catch (e) {}
}
...
function openActionModal(action) {
    ...
    var titles = { ban: '🚫 Ban User', unban: '✅ Unban User', force: '⏏️ Force Logout', unforce: '🔓 Izinkan Login' };
    title.textContent = titles[action] || 'Aksi';
    body.innerHTML = `
        <div class="input-box"><label>Username</label><input type="text" id="actionUsername" placeholder="Ketik username..." maxlength="30"></div>
        <div id="actionUserPreview" style="margin-bottom:12px"></div>
        <button class="btn btn-primary btn-block" onclick="executeAction('${action}')">${titles[action]}</button>
    `;
    modal.classList.add('show');
    setTimeout(function() {
        var inp = document.getElementById('actionUsername');
        if (inp) { inp.focus(); inp.addEventListener('input', function() { previewActionUser(action); }); }
    }, 200);
}
function closeActionModal() { document.getElementById('actionModal').classList.remove('show'); }

function previewActionUser(action) {
    var name = document.getElementById('actionUsername').value.trim();
    var preview = document.getElementById('actionUserPreview');
    if (!name) { preview.innerHTML = ''; return; }
    var u = findUser(name);
    if (!u) { preview.innerHTML = '<div style="padding:10px;background:#fef2f2;border-radius:8px;color:#991b1b;font-size:12px">❌ User tidak ditemukan</div>'; return; }
    var d = calculateDaysLeft(u.expiry_date);
    var dt = d === 999999 ? 'PERMANENT' : (d < 0 ? Math.abs(d) + ' hari lalu' : d + ' hari tersisa');
    preview.innerHTML = '<div style="padding:10px;background:#f0fdf4;border-radius:8px;font-size:12px"><b>✅ ' + esc(u.username) + '</b><br>📱 ' + esc(u.phone || '-') + ' | 🎭 ' + esc(u.role) + ' | 📅 ' + esc(u.expiry_date) + ' (' + dt + ')<br>' + (u.banned ? '<span style="color:#ef4444">⚠️ User sedang dibanned</span>' : '') + (u.forceLogout ? '<span style="color:#f59e0b">⚠️ User sedang di-force logout</span>' : '') + '</div>';
}

function executeAction(action) {
    var name = document.getElementById('actionUsername').value.trim();
    if (!name) return showAlert('Error', 'Masukkan username', 'error');
    var u = findUser(name);
    if (!u) return showAlert('Error', 'User tidak ditemukan', 'error');
    var confirmMsg = {
        ban: 'Ban user "' + name + '"? Mereka tidak bisa login.',
        unban: 'Unban user "' + name + '"?',
        force: 'Force logout "' + name + '"? Mereka tidak bisa login sampai di-unforce.',
        unforce: 'Izinkan "' + name + '" login kembali?'
    };
    showConfirm(confirmMsg[action], function() {
        debounce(action + u.id, function() { return doAction(action, u.id, name); });
    });
}
...
async function doAction(action, id, name) {
    ...
    var patches = {
        ban: { banned: true },
        unban: { banned: false },
        force: { forceLogout: true, logoutTimestamp: Date.now() },
        unforce: { forceLogout: false }
    };
    var successMsg = {
        ban: 'User dibanned!',
        unban: 'User di-unban!',
        force: 'User di-force logout!',
        unforce: 'User bisa login lagi!'
    };
    try {
        await apiCall('users/' + id, 'PATCH', patches[action]);
        showAlert('Berhasil', successMsg[action], 'success');
        await loadUsers(); updateStats(); loadActivity();
...
function switchTab(tab) {
    if (tab === 'adduser') {
        ...
        document.getElementById('subPanelContent').innerHTML = `
            <div class="card" style="background:#fff;border-radius:14px;padding:20px;border:1px solid #e2e8f0">
                <div class="section-title" style="margin-bottom:14px"><i class="fas fa-plus"></i> Tambah User</div>
                <div class="input-box"><label>Username</label><input type="text" id="newUser" placeholder="operator1" maxlength="30"></div>
                <div class="input-box"><label>Nomor</label><input type="text" id="newPhone" placeholder="08123456789" maxlength="20"></div>
                <div class="input-box"><label>Password (min 6 karakter)</label><input type="password" id="newPass" placeholder="min 6 karakter" maxlength="50"></div>
                <div class="input-box"><label>Role</label><select id="newRole"><option>Admin</option><option selected>Operator</option><option>User</option><option>VIP</option><option>Premium</option><option>Trial</option></select></div>
                <div class="input-box"><label>Masa Aktif (MM/DD/YYYY)</label><input type="text" id="newExpiryDate" placeholder="01/28/2026" maxlength="10"></div>
                <button class="btn btn-green btn-block" onclick="addUserNow()"><i class="fas fa-plus"></i> Tambah User</button>
            </div>`;
    } else if (tab === 'users') {
        document.getElementById('adminPanel').style.display = 'none';
        document.getElementById('subPanel').style.display = 'block';
...
async function addUserNow() {
    ...
    try { await apiCall('users', 'POST', { username: u, phone: ph, password: p, role: r, expiry_date: e }); showAlert('Berhasil', 'User ditambahkan!', 'success'); closeSubPanel(); await loadUsers(); updateStats(); loadActivity(); }
    catch (e) { showAlert('Error', e.message, 'error'); }
}

function renderUserList() {
    var users = allUsers.filter(function(u) { return !u.banned; });
    var h = '<div class="card" style="background:#fff;border-radius:14px;padding:20px;border:1px solid #e2e8f0"><div class="section-title" style="margin-bottom:14px"><i class="fas fa-users"></i> List User (' + users.length + ')</div>';
    if (!users.length) { h += '<div class="empty-state">Tidak ada user</div>'; }
    else {
        users.forEach(function(u) {
            var d = calculateDaysLeft(u.expiry_date); var dt = d === 999999 ? 'PERMANENT' : (d < 0 ? Math.abs(d) + ' hari lalu' : d + ' hari tersisa');
            h += '<div style="padding:10px;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:6px;font-size:12px"><b>' + esc(u.username) + '</b> · ' + esc(u.role) + ' · ' + dt + '</div>';
        });
    }
    h += '</div>';
...
async function loadBlockedIPs() {
    try {
        ...
        var h = '<div class="card" style="background:#fff;border-radius:14px;padding:20px;border:1px solid #e2e8f0"><div class="section-title" style="margin-bottom:14px"><i class="fas fa-globe"></i> IP Diblokir (' + allBlockedIPs.length + ')</div>';
        if (!allBlockedIPs.length) { h += '<div class="empty-state">Tidak ada IP diblokir</div>'; }
        else {
            allBlockedIPs.forEach(function(item) {
                h += '<div style="padding:10px;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:6px;font-size:12px;display:flex;justify-content:space-between;align-items:center"><span><b>' + esc(item.ip || 'Unknown') + '</b><br><span style="color:#64748b">' + (item.blocked_at ? new Date(item.blocked_at).toLocaleString('id-ID') : '-') + '</span></span><button class="btn-sm btn-green" onclick="unblockIP(\'' + item.dbKey + '\',\'' + esc(item.ip || 'Unknown') + '\')">Unblock</button></div>';
            });
        }
        h += '</div>';
        document.getElementById('subPanelContent').innerHTML = h;
...
async function unblockIP(dbKey, ip) {
    showConfirm('Unblock IP "' + ip + '"?', async function() {
        try { await apiCall('blocked_ips/' + dbKey, 'DELETE'); showAlert('Berhasil', 'IP di-unblock!', 'success'); loadBlockedIPs(); } catch (e) { showAlert('Error', 'Gagal.', 'error'); }
    });
}
...
function openEditModal() {
    ...
    body.innerHTML = `
        <div class="input-box"><label>Username (cari dulu)</label><input type="text" id="editSearchUser" placeholder="Ketik username..." maxlength="30"></div>
        <div id="editUserPreview" style="margin-bottom:12px"></div>
        <div id="editUserForm" style="display:none">
            <input type="hidden" id="editUserId">
            <div class="input-box"><label>Username Baru</label><input type="text" id="editUsername" maxlength="30"></div>
            <div class="input-box"><label>Nomor</label><input type="text" id="editPhone" maxlength="20"></div>
            <div class="input-box"><label>Password (kosongkan jika tidak diubah)</label><input type="password" id="editPassword" placeholder="min 6 karakter" maxlength="50"></div>
            <div class="input-box"><label>Role</label><select id="editRole"><option>Admin</option><option selected>Operator</option><option>User</option><option>VIP</option><option>Premium</option><option>Trial</option></select></div>
            <div class="input-box"><label>Masa Aktif (MM/DD/YYYY)</label><input type="text" id="editExpiryDate" maxlength="10"></div>
            <button class="btn btn-green btn-block" onclick="saveEditUser()">Simpan</button>
        </div>
    `;
    modal.classList.add('show');
    setTimeout(function() {
        var inp = document.getElementById('editSearchUser');
        if (inp) { inp.focus(); inp.addEventListener('input', previewEditUser); }
    }, 200);
}
function previewEditUser() {
    var name = document.getElementById('editSearchUser').value.trim();
    var preview = document.getElementById('editUserPreview');
    var form = document.getElementById('editUserForm');
    if (!name) { preview.innerHTML = ''; form.style.display = 'none'; return; }
    var u = findUser(name);
    if (!u) { preview.innerHTML = '<div style="padding:10px;background:#fef2f2;border-radius:8px;color:#991b1b;font-size:12px">❌ User tidak ditemukan</div>'; form.style.display = 'none'; return; }
    preview.innerHTML = '<div style="padding:10px;background:#f0fdf4;border-radius:8px;font-size:12px"><b>✅ ' + esc(u.username) + '</b> siap diedit</div>';
...
async function saveEditUser() {
    ...
    try { await apiCall('users/' + id, 'PATCH', data); closeActionModal(); showAlert('Berhasil', 'User diperbarui!', 'success'); await loadUsers(); updateStats(); loadActivity(); }
    catch (e) { showAlert('Error', e.message, 'error'); }
}
