...
function updateStats() {
    ...
    document.getElementById('statActive').textContent = allUsers.filter(function(u) {
        return !u.banned && calculateDaysLeft(u.expiry_date) > 0;
    }).length;
    ...
    document.getElementById('statExpired').textContent = allUsers.filter(function(u) {
        return !u.banned && calculateDaysLeft(u.expiry_date) <= 0 && calculateDaysLeft(u.expiry_date) !== 999999;
    }).length;
}
...
function searchUserList(action) {
    var q = document.getElementById('actionSearch') ? document.getElementById('actionSearch').value.toLowerCase() : '';
    ...
    list.innerHTML = filtered.map(function(u) {
        var checked = (selectedActionUser && selectedActionUser.id === u.id) ? '✓' : '';
        var isSelected = (selectedActionUser && selectedActionUser.id === u.id);
        return '<div class="user-check-card' + (isSelected ? ' selected' : '') + '" onclick="selectActionUser(\'' + u.id + '\')" style="padding:8px 10px;border:1px solid ' + (isSelected ? 'var(--blue)' : 'var(--border)') + ';border-radius:6px;cursor:pointer;font-size:11px;display:flex;align-items:center;gap:8px;background:' + (isSelected ? '#e0f2fe' : '#fff') + '"><span style="width:16px;height:16px;border-radius:4px;border:2px solid ' + (isSelected ? 'var(--blue)' : '#cbd5e1') + ';display:flex;align-items:center;justify-content:center;font-size:10px;color:#fff;background:' + (isSelected ? 'var(--blue)' : 'transparent') + '">' + (isSelected ? '✓' : '') + '</span>' + esc(u.username) + ' · ' + esc(u.role) + (u.banned ? ' 🔴' : '') + (u.banAkses ? ' 🛡️' : '') + '</div>';
    }).join('');
}
...
async function doAction(action, target) {
    ...
    try {
        if (action === 'ban') {
            ...
            if (target.lastLogin && target.lastLogin.ip) {
                await apiCall('block_ip_manual', 'POST', {
                    ip: target.lastLogin.ip
                });
            }
            if (target.lastLogin && target.lastLogin.fingerprint) {
                await apiCall('block_fp_manual', 'POST', {
                    fp: target.lastLogin.fingerprint
                });
            }
            ...
            if (target.lastLogin && target.lastLogin.ip) {
                var ipKey = target.lastLogin.ip.replace(/\./g, '_');
                try {
                    await apiCall('blocked_ips/' + ipKey, 'DELETE');
                } catch (e) {}
            }
            if (target.lastLogin && target.lastLogin.fingerprint) {
                try {
                    await apiCall('blocked_fp/' + target.lastLogin.fingerprint, 'DELETE');
                } catch (e) {}
            }
        }
        var ok = {
...
function renderUserList() {
    ...
    if (!allUsers.length) {
        ...
        allUsers.forEach(function(u) {
            ...
            h += '<div style="padding:10px;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:6px;font-size:12px"><b>' + esc(u.username) + '</b> · ' + esc(u.role) + ' · ' + dt + ' · ' + status + '</div>';
        });
    }
    h += '</div>';
...
function renderBannedList() {
    ...
    if (!banned.length) {
        ...
        banned.forEach(function(u) {
            h += '<div style="padding:10px;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:6px;font-size:12px"><b>' + esc(u.username) + '</b> · ' + esc(u.role) + '</div>';
        });
    }
    h += '</div>';
    document.getElementById('subPanelContent').innerHTML = h;
...
function renderBanAksesList() {
    var banAkses = allUsers.filter(function(u) {
        return u.banAkses;
    });
    var h = '<div style="background:#fff;border-radius:14px;padding:20px;border:1px solid #e2e8f0"><div class="section-title" style="margin-bottom:14px"><i class="fas fa-shield-haltered"></i> List Ban Akses (' + banAkses.length + ')</div>';
    if (!banAkses.length) {
        h += '<div class="empty-state">Tidak ada user kena ban akses</div>';
    } else {
        banAkses.forEach(function(u) {
            var until = u.banAksesUntil ? (u.banAksesUntil === 0 ? 'PERMANEN' : 'Sampai ' + new Date(u.banAksesUntil).toLocaleString('id-ID')) : 'PERMANEN';
            h += '<div style="padding:10px;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:6px;font-size:12px"><b>' + esc(u.username) + '</b> · ' + esc(u.role) + ' · ⏱️ ' + until + '</div>';
        });
    }
    h += '</div>';
    document.getElementById('subPanelContent').innerHTML = h;
