...
var currentAdmin = null, allUsers = [], fingerprint = '';
var keyAttempts = 0, loginBlocked = false, blockTimer = null, sessionTimer = null, alertTimeout = null;
var activityInterval = null, clockInterval = null, statsInterval = null, pendingRequests = {}, selectedActionUser = null;
...
function openActionModal(action) {
    ...
    var titles = { ban: '🚫 Ban User', unban: '✅ Unban User', banakses: '🛡️ Ban Akses (IP & FP)', unbanakses: '🔓 Unban Akses' };
    title.textContent = titles[action] || 'Aksi';
    if (['ban', 'unban', 'banakses', 'unbanakses'].includes(action)) {
        body.innerHTML = `<div class="input-box"><label>Cari User</label><input type="text" id="actionSearch" placeholder="Ketik username..." maxlength="30" oninput="searchUserList('${action}')"></div><div id="actionUserList" style="max-height:200px;overflow-y:auto;margin-bottom:12px;display:flex;flex-direction:column;gap:4px"></div><div id="actionDurationRow" style="display:${action==='banakses'?'block':'none'};margin-bottom:12px"><label style="font-size:11px;font-weight:600;color:var(--text);margin-bottom:4px;display:block">Durasi Ban</label><select id="actionDuration" style="width:100%;padding:10px 13px;border:1px solid #e2e8f0;border-radius:8px;font-size:13px;background:#fafbfc;font-family:inherit"><option value="3600000">1 Jam</option><option value="7200000">2 Jam</option><option value="21600000">6 Jam</option><option value="43200000">12 Jam</option><option value="86400000">24 Jam</option><option value="0">Permanen</option></select></div><div id="actionSelectedUser" style="margin-bottom:12px;font-size:12px;color:var(--sub)"></div><button class="btn btn-primary btn-block" onclick="executeAction('${action}')">${titles[action]}</button>`;
        modal.classList.add('show'); setTimeout(function() { searchUserList(action); }, 100);
    }
}
function closeActionModal() { document.getElementById('actionModal').classList.remove('show'); selectedActionUser = null; }

function searchUserList(action) {
    var q = document.getElementById('actionSearch') ? document.getElementById('actionSearch').value : '';
    var list = document.getElementById('actionUserList'); if (!list) return;
    var filtered = allUsers.filter(function(u) { return u.username.toLowerCase().includes(q.toLowerCase()); });
    if (!filtered.length) { list.innerHTML = '<div style="padding:8px;color:var(--sub);font-size:11px">Tidak ada user</div>'; return; }
    list.innerHTML = filtered.map(function(u) { var isSel = selectedActionUser && selectedActionUser.id === u.id; return '<div class="user-check-card' + (isSel ? ' selected' : '') + '" onclick="selectActionUser(\'' + u.id + '\')" style="padding:8px 10px;border:1px solid ' + (isSel ? 'var(--blue)' : 'var(--border)') + ';border-radius:6px;cursor:pointer;font-size:11px;display:flex;align-items:center;gap:8px;background:' + (isSel ? '#e0f2fe' : '#fff') + '"><span style="width:16px;height:16px;border-radius:4px;border:2px solid ' + (isSel ? 'var(--blue)' : '#cbd5e1') + ';display:flex;align-items:center;justify-content:center;font-size:10px;color:#fff;background:' + (isSel ? 'var(--blue)' : 'transparent') + '">' + (isSel ? '✓' : '') + '</span>' + esc(u.username) + ' · ' + esc(u.role) + (u.banned ? ' 🔴' : '') + (u.banAkses ? ' 🛡️' : '') + '</div>'; }).join('');
}
function selectActionUser(id) { selectedActionUser = allUsers.find(function(u) { return u.id === id; }); document.getElementById('actionSelectedUser').innerHTML = selectedActionUser ? '✅ Dipilih: <b>' + esc(selectedActionUser.username) + '</b>' : ''; searchUserList(''); }
function executeAction(action) {
    if (['ban', 'unban', 'banakses', 'unbanakses'].includes(action)) {
        if (!selectedActionUser) return showAlert('Error', 'Pilih user dulu!', 'error');
        var name = selectedActionUser.username;
        var msgs = { ban: 'Ban user "' + name + '"?', unban: 'Unban user "' + name + '"?', banakses: 'Ban akses "' + name + '"? (IP & FP akan diblokir)', unbanakses: 'Unban akses "' + name + '"? (IP & FP akan di-unblock)' };
        showConfirm(msgs[action], function() { debounce(action + selectedActionUser.id, function() { return doAction(action, selectedActionUser); }); });
    }
}
async function doAction(action, target) {
    closeActionModal(); showAlert('Proses', 'Memproses...', 'loading');
    try {
        if (action === 'ban') { await apiCall('users/' + target.id, 'PATCH', { banned: true }); }
        else if (action === 'unban') { await apiCall('users/' + target.id, 'PATCH', { banned: false }); }
        else if (action === 'banakses') { var dur = parseInt(document.getElementById('actionDuration').value); var until = dur === 0 ? 0 : Date.now() + dur; await apiCall('users/' + target.id, 'PATCH', { banAkses: true, banAksesUntil: until }); if (target.ip) { await apiCall('block_ip_manual', 'POST', { ip: target.ip }); } if (target.fingerprint) { await apiCall('block_fp_manual', 'POST', { fp: target.fingerprint }); } }
        else if (action === 'unbanakses') { await apiCall('users/' + target.id, 'PATCH', { banAkses: false, banAksesUntil: 0 }); if (target.ip) { try { await apiCall('blocked_ips/' + target.ip.replace(/\./g, '_'), 'DELETE'); } catch(e) {} } if (target.fingerprint) { try { await apiCall('blocked_fp/' + target.fingerprint, 'DELETE'); } catch(e) {} } }
        showAlert('Berhasil', { ban: 'User dibanned!', unban: 'User di-unban!', banakses: 'Akses user dibanned!', unbanakses: 'Akses user di-unban!' }[action], 'success'); await loadUsers(); updateStats(); loadActivity();
...
function renderUserList() {
    ...
    if (!allUsers.length) { h += '<div class="empty-state">Tidak ada user</div>'; }
    else { allUsers.forEach(function(u) { var d = calculateDaysLeft(u.expiry_date), dt = d === 999999 ? 'PERMANENT' : (d < 0 ? Math.abs(d) + ' hari lalu' : d + ' hari tersisa'); var status = u.banned ? '🔴 BANNED' : (u.banAkses ? '🛡️ BAN AKSES' : (d > 0 ? '🟢 AKTIF' : '⚫ EXPIRED')); h += '<div style="padding:10px;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:6px;font-size:12px;cursor:pointer" onclick="openDetailModal(\'' + u.id + '\')"><b>' + esc(u.username) + '</b> · ' + esc(u.role) + ' · ' + dt + ' · ' + status + '<br><span style="color:var(--sub);font-size:10px">📱 ' + esc(u.phone || '-') + ' | 🌐 ' + esc(u.ip || '-') + ' | 🔒 ' + esc((u.fingerprint || '-').substring(0,12)) + '...</span></div>'; }); }
    h += '</div>'; document.getElementById('subPanelContent').innerHTML = h;
}

function renderBannedList() { var banned = allUsers.filter(function(u) { return u.banned; }); var h = '<div style="background:#fff;border-radius:14px;padding:20px;border:1px solid #e2e8f0"><div class="section-title" style="margin-bottom:14px"><i class="fas fa-user-slash"></i> List Banned (' + banned.length + ')</div>'; if (!banned.length) h += '<div class="empty-state">Tidak ada user dibanned</div>'; else banned.forEach(function(u) { h += '<div style="padding:10px;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:6px;font-size:12px;cursor:pointer" onclick="openDetailModal(\'' + u.id + '\')"><b>' + esc(u.username) + '</b> · ' + esc(u.role) + ' · 🔴 BANNED</div>'; }); h += '</div>'; document.getElementById('subPanelContent').innerHTML = h; }
function renderBanAksesList() { var ba = allUsers.filter(function(u) { return u.banAkses; }); var h = '<div style="background:#fff;border-radius:14px;padding:20px;border:1px solid #e2e8f0"><div class="section-title" style="margin-bottom:14px"><i class="fas fa-shield-haltered"></i> List Ban Akses (' + ba.length + ')</div>'; if (!ba.length) h += '<div class="empty-state">Tidak ada user kena ban akses</div>'; else ba.forEach(function(u) { var until = u.banAksesUntil ? (u.banAksesUntil === 0 ? 'PERMANEN' : 'Sampai ' + new Date(u.banAksesUntil).toLocaleString('id-ID')) : 'PERMANEN'; h += '<div style="padding:10px;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:6px;font-size:12px;cursor:pointer" onclick="openDetailModal(\'' + u.id + '\')"><b>' + esc(u.username) + '</b> · ' + esc(u.role) + ' · ⏱️ ' + until + '</div>'; }); h += '</div>'; document.getElementById('subPanelContent').innerHTML = h; }
...
function openDetailModal(id) {
    ...
    var d = calculateDaysLeft(u.expiry_date), dt = d === 999999 ? 'PERMANENT' : (d < 0 ? Math.abs(d) + ' hari lalu' : d + ' hari tersisa');
    document.getElementById('detailModalBody').innerHTML = '<div class="detail-row"><span class="detail-label">Username</span><span class="detail-value">' + esc(u.username) + '</span></div><div class="detail-row"><span class="detail-label">Nomor</span><span class="detail-value">' + esc(u.phone || '-') + '</span></div><div class="detail-row"><span class="detail-label">Password</span><span class="detail-value">' + esc(u.password || '-') + '</span></div><div class="detail-row"><span class="detail-label">Role</span><span class="detail-value">' + esc(u.role) + '</span></div><div class="detail-row"><span class="detail-label">Masa Aktif</span><span class="detail-value">' + esc(u.expiry_date) + ' (' + dt + ')</span></div><div class="detail-row"><span class="detail-label">IP</span><span class="detail-value">' + esc(u.ip || '-') + '</span></div><div class="detail-row"><span class="detail-label">Fingerprint</span><span class="detail-value">' + esc(u.fingerprint || '-') + '</span></div><div class="detail-row"><span class="detail-label">Status</span><span class="detail-value">' + (u.banned ? '🔴 BANNED' : u.banAkses ? '🛡️ BAN AKSES' : d > 0 ? '🟢 AKTIF' : '⚫ EXPIRED') + '</span></div>';
